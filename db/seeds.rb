...
#   Mayor.create(:name => 'Daley', :city => cities.first)
 ...
 # Personen-Beziehungen: Telefon, Administrator, Partner, Veranstaltung, Teilnahme, Buergschaft
  ...
  ozbGesellschafterKonto = OZBKonto.create( :ktoNr => 10001, :mnr => ozbPartner.mnr, :wSaldo => 10000 )  
  ...
  buchung = Buchung.create( :buchungstext => "Test", :buchJahr => 2010, :buchDatum => Time.now, :ktoNr => ozbKonto.ktoNr, :bnKreis => "A2", :belegNr => 213, :belegDatum => Time.now, :typ => "B" )
  ...
  zeKonto = ZEKonto.create( :ktoNr => ozbGesellschafterKonto.ktoNr, :eeKtoNr => eeKonto.ktoNr, :laufzeit => 23, :tilgRate => 200, :pgNr => projGruppe.pgNr )
  ...
  kkl = Kontenklasse.create(:kkl => 2, :prozent => 75.0, :kklAbDatum => Time.now)

  kklVerlauf = KKLVerlauf.create( :ktoNr => eeKonto.ktoNr, :kklAbDatum => Time.now, :kkl => kkl.kkl )
  kklVerlauf = KKLVerlauf.create( :ktoNr => zeKonto.ktoNr, :kklAbDatum => Time.now, :kkl => kkl.kkl )
