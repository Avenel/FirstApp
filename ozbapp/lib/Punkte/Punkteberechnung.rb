class Punkteberechnung
  ...
  def self.get_days_in_account_classes(account_class_changes, date_begin, date_end)
    ...
    account_class_changes.each_with_index do |account_class_change, i|
      from_date = i == 0 ? date_begin : account_class_change.KKLAbDatum
      to_date = account_class_changes[i+1].nil? ? date_end : account_class_changes[i+1].KKLAbDatum

      days_in_account_classes[account_class_change.KKL] = count_days_exact(from_date, to_date)

      if (i > 0) # add one day between an account class change.
        days_in_account_classes[account_class_change.KKL] = days_in_account_classes[account_class_change.KKL] + 1
      end
    end
  ...
  def self.count_days_exact(first_time, second_time)
    days = ((second_time.to_time - first_time.to_time) / 1.day).to_i
